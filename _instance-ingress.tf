data "template_file" "k8s-ingress" {
...
resource "aws_instance" "k8s-ingress" {
  count                       = "1"
  ami                         = "${lookup(var.amis, var.region)}"
  instance_type               = "t2.medium"
  subnet_id                   = "${aws_subnet.k8s-public.id}"
  key_name                    = "${var.aws_key_name}"
  private_ip                  = "${var.subnet_prefix}${var.ingress_ip_suffix}"
  user_data                   = "${data.template_file.k8s-ingress.rendered}"
  ...
  vpc_security_group_ids = [
    "${aws_security_group.k8s-ingress.id}",
  ]
